...
private date: any = {};
    ...
    if(this.offre.titre==""){
      ...
      this.service.addOffre(this.offre).subscribe(res=>{
        if(res.error==true){
          ...
          if(this.offre.id){
            console.log("test---",this.offre.test);
      ...
      err => {
        this.rep.message="une error dans le serveur mail essayer une autre fois";
        this.rep.error=true;
        this.show = false;
      })
    }
  ...
  getOffres(){
    ...
    this.service.getOffres(this.option).subscribe(res=>{
      this.offres=res;
      for(let o of this.offres.content) {
        o.verif = false;
        if(o.test == null){
          o.verif = true;
        }
  ...
  matching(id,verif){
    if(!verif){
      ...
      this.sharedSrvice.subject.next({toastr:{type:"error",message:"Vous devez ajouter un test avant d'inviter les candidat"}});
    }
  }
  setEdition(o){
    this.rep={};
